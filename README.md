...
This project implements a control system for the Kinematic Module on C219/W211. For testing i used this Code with an Arduino to control the (TLE4207 full-bridge driver IC) found on the kinematic PCB. The system supports automated initialization, end-position detection, recovery after power loss, and a robust fallback mechanism if the drive cannot reach its intended limit. For making this project work you should cut three INPUT lines below the TLE4207 connecting to the Freescale IC (Big IC in the middle of the Board). Connections you should cut are INH, IN1, IN2. You can get 5V and GND from the TLE4268 IC (a DC DC 12V to 5V converter) or you can find 5V and GND around TLE4207. 
...
- Pressing the input button **toggles** the motor action:
  ...
  - While in Motion, Button Press is ignored

### Fallback: Failed Motion Recovery
- If the motor is unable to reach the expected end stop (e.g., obstruction or resistance):
  - The system performs **two reverse attempts** (can be changed in config) to reach the opposite end.
...
- If power is lost during motion (e.g., the car is turned off mid-action):
  - On the next boot, the controller will **attempt to close the drive automatically** (can be changed in config).
...
### Components
- Arduino (e.g., Nano, Pro Mini, or UNO), you can use a ATTiny45 or similar/smaller later like me (chnage pinout in config according to your ic)
